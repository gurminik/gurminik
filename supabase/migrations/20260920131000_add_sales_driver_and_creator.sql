-- Older sales remain readable: existing plate defaults are retained and
-- historic driver/creator values are unknown rather than guessed.
alter table public.sales
  add column if not exists driver_name text not null default '',
  add column if not exists created_by uuid default auth.uid();

-- Existing sales RLS and owner guard apply to these columns as before.
-- The owner/date and product indexes already cover current paged reads;
-- client-side driver searching does not benefit from an extra database index.

-- Keep JSON restores compatible with both the older sales format and the new
-- driver/plate fields. Existing rows are never overwritten during restore.
...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(payload->'sales', '[]'::jsonb)) loop
    ...
    insert into public.sales(id, owner_id, product_id, buyer_name, driver_name, vehicle_plate, quantity_kg, unit_sale_price, transaction_at, status)
    values (
      sale_id, uid, resolved_product,
      coalesce(nullif(item->>'buyer', ''), nullif(item->>'buyer_name', ''), 'Alıcı'),
      coalesce(item->>'driver', item->>'driver_name', ''),
