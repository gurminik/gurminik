...
const migration = readFileSync(
  new URL(
    "../supabase/migrations/20260912093750_add_accounts_favorites_and_ranking_permissions.sql",
    import.meta.url,
  ),
  "utf8",
);
...
assert.match(page, /products\.map\(\s*\(product\)\s*=>/);
...
const rows = Object.values(
  purchases.reduce((acc, item) => {
    acc[item.person] ??= {
      person: item.person,
      total: 0,
      byProduct: Object.fromEntries(products.map((p) => [p, 0])),
    };
...
assert.equal(
  [...rows].sort((a, b) => b.byProduct.limon - a.byProduct.limon)[0].person,
  "Ali",
);
assert.equal(
  [...rows].sort((a, b) => a.byProduct.limon - b.byProduct.limon)[0].person,
  "Murat",
);

console.log(
  "GURMİNİK özellik kontrolleri başarılı: menü, sıralama, kişi önizleme, telefon, favori hızlı alış, cari hesap ve PDF.",
);
